...
type PublicThemeOptions = {
  includeBookStyles?: boolean;
};

export function ensurePublicTheme({ includeBookStyles = false }: PublicThemeOptions = {}): void {
  if (typeof document === "undefined") {
    return;
  }

  if (!document.querySelector('script[data-qlx="public-tailwind-cdn"]')) {
    const script = document.createElement("script");
    script.src = TAILWIND_CDN_URL;
    script.async = true;
    script.dataset.qlx = "public-tailwind-cdn";
    document.head.appendChild(script);
  }

  if (!document.querySelector('script[data-qlx="public-tailwind-config"]')) {
    const configScript = document.createElement("script");
    configScript.type = "text/javascript";
    configScript.dataset.qlx = "public-tailwind-config";
    configScript.innerHTML = TAILWIND_CONFIG_SCRIPT;
    document.head.appendChild(configScript);
  }

  if (!document.querySelector('style[data-qlx="public-tailwind-base"]')) {
    const styleEl = document.createElement("style");
    styleEl.type = "text/tailwindcss";
    styleEl.dataset.qlx = "public-tailwind-base";
    styleEl.innerHTML = PUBLIC_BASE_STYLES;
    document.head.appendChild(styleEl);
  }

  if (includeBookStyles && !document.querySelector('style[data-qlx="public-tailwind-book"]')) {
    const styleEl = document.createElement("style");
    styleEl.type = "text/tailwindcss";
    styleEl.dataset.qlx = "public-tailwind-book";
    styleEl.innerHTML = PUBLIC_BOOK_STYLES;
    document.head.appendChild(styleEl);
  }

  if (!document.querySelector('link[data-qlx="material-symbols"]')) {
    const linkEl = document.createElement("link");
    linkEl.rel = "stylesheet";
    linkEl.href = MATERIAL_SYMBOLS_URL;
    linkEl.dataset.qlx = "material-symbols";
